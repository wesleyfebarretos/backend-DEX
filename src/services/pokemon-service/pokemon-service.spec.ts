...
describe("Pokemon Service Test", () => {
	...
	describe("CREATE", () => {
      ...
      >;
			...
			const spriteFindOneMethod =
        spriteRepository.findOne as jest.MockedFunction<
          typeof spriteRepository.findOne
        >;
        ...
        >;
			...
			const typeFindOneByMethod =
        typeRepository.findOneBy as jest.MockedFunction<
          typeof typeRepository.findOneBy
        >;

			const createMethod = (
        pokemonRepository.create as jest.MockedFunction<
          typeof pokemonRepository.create
        >
			).mockReturnValue({
				id: 1,
			...
			const saveMethod = (
        pokemonRepository.save as jest.MockedFunction<
      ...
      >;
			...
			await expect(pokemonService.create("test", [], [], [])).rejects.toThrow(
				new BadRequestError(`Pokemon ${poke1.name} already exist`)
			);
		});
      ...
      >;
			...
			await expect(
				pokemonService.create("test", [1, 2, 3, 4], [], [])
			).rejects.toThrow(
				new BadRequestError(
      ...
      >;
			...
			await expect(
				pokemonService.create("test", [1, 2], [], [])
			).rejects.toThrow(
				new BadRequestError(
      ...
      >;
			...
			await expect(
				pokemonService.create("test", [1, 2], [1, 2], [])
			).rejects.toThrow(
				new BadRequestError(
      ...
      >;
			...
			await expect(
				pokemonService.create("test", [1, 2], [1, 2], sprites)
			).rejects.toThrow(
				new BadRequestError(
      ...
      >;
        ...
        >;
			...
			await expect(
				pokemonService.create("test", [1, 2], [1, 2], sprites)
			).rejects.toThrow(new BadRequestError("img: a.com.br already exist"));
		});
	});
	...
	describe("UPDATE", () => {
      ...
      >;
        ...
        >;
			...
			await expect(
				pokemonService.updateName("whatever", poke1.name)
			).rejects.toThrow(new NotFoundError("Pokemon not found"));
		});
        ...
        >;
			...
			await expect(
				pokemonService.updateAbilities([], poke1.name)
			).rejects.toThrow(new NotFoundError("Pokemon not found"));
		});
        ...
        >;
			...
			await expect(pokemonService.updateTypes([], poke1.name)).rejects.toThrow(
				new NotFoundError("Pokemon not found")
			);
		});
        ...
        >;
			...
			await expect(
				pokemonService.updateSprites([], poke1.name)
			).rejects.toThrow(new NotFoundError("Pokemon not found"));
		});
	});
	...
	describe("DELETE", () => {
      ...
      >;
        ...
        >;
			...
			await expect(pokemonService.delete(`${poke1.id}`)).rejects.toThrow(
				new NotFoundError("Pokemon not found")
			);
		});
	});
