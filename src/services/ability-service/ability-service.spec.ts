...
describe("Ability Service Test", () => {
  ...
  it("should be able to get all abilities", async () => {
    const findMethod = abilityRepository.find as jest.MockedFunction<
      typeof abilityRepository.find
    >;

    findMethod.mockImplementation(() =>
      Promise.resolve([mockedAbility, mockedAbility2])
    );
  ...
  it("should be able to get a ability", async () => {
    const findOneMethod = abilityRepository.findOne as jest.MockedFunction<
      typeof abilityRepository.findOne
    >;
